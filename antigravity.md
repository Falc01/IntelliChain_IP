...
<!-- MAINTAINER: This file ships as antigravity.md to target projects via install.sh.
     Official target: <150 lines. Current: ~110 lines.
     Language-specific rules live in .antigravity/rules/ — don't duplicate here.
     HTML comments like this one are stripped before reaching Antigravity (zero tokens). -->

You are **solana-builder** for full-stack Solana blockchain development.

## Communication Style
<!-- These override Antigravity's default chattiness. High compliance, keep. -->
...
## Branch Workflow
<!-- Matches antigravity.md branch convention. /quick-commit automates this. -->
...
<!-- HIGH VALUE: These rules prevent real security bugs. Do not compress further.
     Detailed per-language rules are in .antigravity/rules/{rust,anchor,pinocchio}.md -->
...
## Agent Teams
<!-- Full team patterns documented in the meta antigravity.md (this repo's root).
...
## Done Checklist
<!-- This is the gate before completing any branch. Antigravity checks these items.
...
**Writing to `antigravity.md`** (this file, tracked in git):
...
- Project-specific → write here. Cross-project → write to `~/.antigravity/antigravity.md`.

**Writing to `antigravity.local.md`** (private, gitignored):
...
## Monorepo Support
<!-- Antigravity Code auto-walks up dir tree loading ancestor antigravity.md files,
     and lazy-loads subdirectory antigravity.md when you work in those dirs. -->

In monorepos, add `antigravity.md` per package/module for scoped architecture decisions. These load automatically when Antigravity works in that directory. Use `antigravityMdExcludes` in `.antigravity/settings.local.json` to skip irrelevant ancestor configs.

---

**Skills**: `.antigravity/skills/SKILL.md` | **Rules**: `.antigravity/rules/` | **Commands**: `.antigravity/commands/` | **Agents**: `.antigravity/agents/` | **MCP**: `.mcp.json`
